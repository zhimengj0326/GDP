...
def read_dataset(name, label=None, sensitive_attribute=None, fold=None):
    if name == 'crimes':
        ...
        z_name = sensitive_attribute if sensitive_attribute is not None else ['racepctblack', 'agePct12t29']
...
def read_crimes(label='ViolentCrimesPerPop', sensitive_attribute=['racepctblack', 'agePct12t29'], fold=1):
...
def load_adult(nTrain=None, scaler=True, shuffle=False):
    ...
    to_protect_index = [0, 4] ### [age, education-num]
    ...
    ### normalize age 
    # print(f'to_protect={to_protect.shape}')
    norm_max = np.maximum(np.amax(to_protect, axis=0), np.amax(to_protect_test, axis=0))
    norm_min = np.minimum(np.amin(to_protect, axis=0), np.amin(to_protect_test, axis=0))
